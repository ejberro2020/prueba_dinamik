...
class WPBS_Calendar_Outputter {
	...
	public function get_display() {
		...
		$calendar_html_data .= 'data-currency="' . wpbs_get_currency() . '" ';
	...
	protected function get_display_day( $year, $month, $day ) {
		...
		if( ! empty( $day ) ) {
			...
			// If today
			if( $is_today && $this->args['highlight_today'] ) {
				$legend_item_id_icon   = 'today';
        ...
        $ical_changeover = is_admin() && $ical_event && $ical_event->get('meta') == 'ical-changeover' ? 'wpbs-ical-changeover' : '';
		 ...
		 *
		 */
		$output .= '<div class="wpbs-date ' . ( ! empty( $legend_item_id_icon ) && is_numeric( $legend_item_id_icon ) ? 'wpbs-legend-item-' . $legend_item->get('id') : '' ) . ' ' . ( $is_today ? 'wpbs-date-today' : '' ) . ' ' . ( empty( $day ) ? 'wpbs-gap' : '' ) . ' '. ($is_bookable ? 'wpbs-is-bookable' : '' ). ' ' . $ical_changeover . '" ' . ( ! empty( $day ) ? ( 'data-year="' . esc_attr( $year ) . '" data-month="' . esc_attr( $month ) . '" data-day="' . esc_attr( $day ) . '"' ) : '' ) . ' data-price="'.$price.'">';
			...
			$output .= '<div class="wpbs-date-inner">' . ( ! empty( $day ) ? '<span class="wpbs-date-number">' . apply_filters('wpbs_calendar_outputter_date_number', $day, $event) . '</span>' : '' ) . '</div>';
	...
	protected function get_display_day_tooltip( $year, $month, $day ) {
		...
		$output = '<div class="wpbs-tooltip">';
			$output .= '<strong>' . date( 'd', mktime( 0, 0, 0, $month, $day, $year ) ) . ' ' . wpbs_get_month_name( $month, $this->args['language'] ) . ' '. date( 'Y', mktime( 0, 0, 0, $month, $day, $year ) ) . '</strong>';
			$output .= $tooltip;
		$output .= '</div>';
