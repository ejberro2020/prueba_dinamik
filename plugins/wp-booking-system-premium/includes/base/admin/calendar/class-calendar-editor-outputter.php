...
class WPBS_Calendar_Editor_Outputter
{
    ...
    protected function get_display_date_header()
    {
        ...
        $output .= '<div class="wpbs-calendar-date-tooltip-header">' . __('Tooltip', 'wp-booking-system') . wpbs_get_output_tooltip(__('You can use the tooltip field of the date to add information that you wish users to see when they hover over the calendar date in the front-end part of your website.', 'wp-booking-system')) . '</div>';
    ...
    protected function get_display_date($year, $month, $day)
    {
        ...
        if (!is_null($ical_event)) {
            ...
            $output .= apply_filters('wpbs_calendar_editor_columns_before_description', '', $year, $month, $day, $event, $data, $this->default_price, $this->default_inventory);
    ...
    protected function get_display_date_legend_item($year, $month, $day, $event, $data)
    {
        ...
        $ical_changeover = $event && $event->get('meta') == 'ical-changeover' ? 'wpbs-ical-changeover' : '';
